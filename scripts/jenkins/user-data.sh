...
docker run --name jenkins -d --network=devops --user=root -v /root/jenkins:/var/jenkins_home -p 8080:8080 -v $(which docker):/usr/bin/docker -v /var/run/docker.sock:/var/run/docker.sock -v /root:/root --restart always jenkins/jenkins:lts
...
docker exec jenkins apt update -y
docker exec jenkins apt install -y maven
